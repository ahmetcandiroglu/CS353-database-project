...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
      ...
      if( empty($firstName_err) &&  empty($lastName_err) &&  empty($username_err) &&  empty($userEmail_err) &&  empty($password_err) &&  empty($confirm_password_err) &&  empty($city_err) &&  empty($date_err) &&  empty($month_err) &&  empty($year_err) ){
          ...
          $sql = "INSERT INTO user_table(username, user_firstName, user_lastName, user_email, user_password, user_birthdate, user_gender, city, user_profileType) VALUES ('$username', '$firstName', '$lastName', '$userEmail', '$password', '$userBirthday', '$gender', '$city', '$type' )";
          $query = mysqli_query($db, $sql);
